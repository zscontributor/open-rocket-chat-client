...
**Required status checks — everything CI runs.** Format/lint/typecheck/build, unit tests, the
dependency audit and the branch policy. There is nothing in CI that is not required, because
**Playwright no longer runs here at all** — it is a local suite, documented in
[CONTRIBUTING.md](../CONTRIBUTING.md). Standing up Rocket.Chat, MongoDB, the gateway and a browser on
a hosted runner cost more than twenty minutes a run, which is long enough that people stop reading
the answer.

That does leave a gap worth naming: the assertion that no Rocket.Chat token is reachable from the
page now has nothing enforcing it on the way in. It is the property this whole architecture exists
for, so a change anywhere near sessions, storage or the SDK deserves a local `pnpm run test:e2e` and
a note in the pull request saying it was run.

If it is ever worth bringing back, the cheap shape is a separate workflow on `workflow_dispatch` plus
a nightly `schedule` — off the pull request path, so a slow suite costs nobody their afternoon.
