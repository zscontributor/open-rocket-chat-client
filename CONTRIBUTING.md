...
CI runs the same commands on every pull request and the checks are required, so running them locally
only saves you a round trip.

`pnpm run test:e2e` assumes the stack the Compose file seeds. Three variables override that, and all
three have defaults, so a standard local setup needs none of them:
