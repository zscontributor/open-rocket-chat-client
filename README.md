...
**It runs locally only.** Standing that stack up on a hosted runner cost over twenty minutes a run, so CI covers formatting, linting, types, the build, unit tests and a dependency audit, and leaves this suite to you. Run it before opening a pull request that touches anything it covers.
