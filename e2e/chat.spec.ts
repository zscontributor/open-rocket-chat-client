...
test('delivers a message from another session over the realtime channel', async ({ page, browser }) => {
  ...
  const composer = otherPage.getByRole('textbox', { name: /^Message / });
  await composer.fill(text);
  await composer.press('Enter');
...
test('hands the composer back when every uploading file is removed', async ({ page }) => {
  ...
  await page.getByRole('textbox', { name: /^Message / }).fill('with a caption');
  ...
  await expect(page.getByRole('textbox', { name: /^Message / })).toHaveValue('with a caption');
});
